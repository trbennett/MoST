...
%% Get files in directory that is being changed to with getFiles, and change to first file in list
function [vs fh] = changeDir(vs)
    vs=getFiles( vs); % Get the files in the current directory
    [vs fh] = changeFile(vs, 1);
end

%% Change the current files being examined to that selected by user
...
%% Get a list of the files in the given data directory and return it
function vs = getFiles(vs)
    ...
    for idx=1:size(mat,1)
        ...
        files(idx).f={};
        files(idx).vf={};
        %file name & path of the timestamp file
        files(idx).vtstamp={};
        files(idx).af=[];
        for mote = vs.motes'
        ...
        for cam=1:vs.ncams
            ...
            %timestamp file
            vfnamet = sprintf('%s%sm%04i_s%02i_m%02i_c%02i.avi.txt', vdir, filesep, [mat(idx,:) cam]);
            files(idx).vtstamp={files(idx).vtstamp{:} vfnamet};
        end
...
%% Create the figure we do all our drawing in
function fh=createFig(vs)
    ...
    set(fh,'pointer','fullcrosshair'); % Crosshair pointer for figure
...
%% Function to resize the video for the data
...
%% Function to handle closing of the tool
...
%% Experiment selection menu, calls changeFile for file selected
...
%% Function for plotting data on the graphs
...
%          data = data(sensor_start_idx:end,:);
    ...
    %--- Read the timestamp file, create frame list (X) based on the duration of each frame ---
    for idx=1:numel(vs.vid_h)
        vtstamp = vs.files(vs.sel_file).vtstamp{idx};   % Obtain corresponding file for video timestamps
        stampdata=importdata(vtstamp);                  % Read video timestamps
        rows=numel(stampdata);                          % Get number of rows in video timestamps
        start=round((stampdata(rows)-stampdata(1))/5);  % Get length of video from timestamps
        start=data_len-start+1;                         % Subtract video length from min data length

        framedur = diff(stampdata);%stampdata(2:end,1)-stampdata(1:end-1,1); % Obtain length of video frames based on timestamp differences between frames

        % The below loop and following two lines make each frame have a
        % duration that is a multiple of 5
        for i=2:rows-1                                  % For each pair of timestamps
            temp=mod(framedur(i-1),5);                  % Get duration mod 5 of lower
            framedur(i-1)=framedur(i-1)-temp;           % Subtract the remainder
            framedur(i)=framedur(i)+temp;               % And add it to the higher timestamp
        end
        temp=mod(framedur(rows-1),5);                   % Get mod 5 of last frame's duration
        framedur(rows-1)=framedur(rows-1)-temp;         % Perform above for this special case
        
        
        X=rand(data_len,1);                             % This variable will become the framelist
        
        if(start <= 0) % HCM - temporary fix must be removed!
            start = 1;
        end
        it=start;                                       % Iterator variable
        for j=1:rows-1                                  % For each frame
            count=framedur(j)/5;                        % Get duration/5 (or # of samples)
            for k=1:count                               % For each sample in frame
            X(it)=double(k)/double(count)+j;            % X(it) = (sample #/#samples in frame) + frame #
            it=it+1;                                    % Move iterator to next spot in array
            end
        end
    ...
    % Doesn't do anything for new version where framelist is same for each
    % data set
    %{  
    flist_all = [];
    for data_idx = 1:numel(adata)
        ...
        %copy frame list created from the timestamps above at line 537 to 567
        fl = X;
...
%% Handle choosing of individual sensors
...
%% Handle choosing of individual modalities
...
%% Function to handle zooming out after zooming in
...
%% Function to handle pressing of Dir button to change data directory
...
%% The field that controls the size of the moving average filter applied to the data
...
%% Align data positions with frame numbers based on framelist
...
%% Align video frames to data samples
...
%% Set position of the marker on user click
...
%% Set position of the maker on user click for case with no video
...
%% Get the current position of the user marker
...
%% Validate that the position of the marker is within limits of data samples length
...
%% Set the position of the marker based on user click
...
%% Set the time in video based on marker position and user click
...
%% Set the current frame of the video based on input frame #
function setVideoFrame(frame, cam)
    ...
    if(isempty(frame_cache) || frame < frame_cache.first || frame > frame_cache.last)
        ...
        last_frame = vs.framelist{1}; % HCM
        last_frame = last_frame(length(last_frame), 2);
        frame_cache.last = min(frame+sf, last_frame);
...
%% Advance to the next frame in the video based on position
...
%% A moving average filter for the data. Size of window controlled by user input
...
%% Control how the axis is displayed
function setAxis()
    fh = getfh();
    vs = get(fh,'UserData');
...
%% Function to handle all the cases where keys are pressed by user
...
%% Handle click and drag motions of mouse on data
...
%% The scroll bar for when zoomed in on a data segment
...
%% Update the scrollbar based on the zoom and step
...
%% Get the color of the current annotation based on color list annote_colors
...
function saveAnnotations()
    ...
    % HCM - if using space to place markers, get floating point values!
    % Must round to prevent this.
    an = round(an);
    dlmwrite(afname,an,',');
