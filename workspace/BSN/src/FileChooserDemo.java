	...
	public void actionPerformed(ActionEvent e) {
				...
				try {
					for (int w = 0; w < index.size() - 1; w = w + 2) {
						...
						e1 = new extract_Data(index.elementAt(w),
								index.elementAt(w + 1), files_extract, primo,w,(index.size()),log);
						primo = false;
					}
				} catch (IOException ex) {
					ex.printStackTrace();
					log.append("Extract failed. Check that directory structure is unaltered\n(in same folder as \"split\", there should be a folder named \"annotated\"");
				}
			} else {
				log.append("Extract command cancelled by user." + newline);
			}
			log.setCaretPosition(log.getDocument().getLength());
			//log.append("COMPLETE" + newline);
		} 
		// Handle files_index button action.
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				try {
					extract_Index ei = new extract_Index(file, log);
					index = ei.getIndices();
				} catch (IOException e1) {
					e1.printStackTrace();
					log.append("Indexing failed.\n");
				}
