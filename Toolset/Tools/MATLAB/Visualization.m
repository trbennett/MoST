...
function fh=createFig(vs)
    ...
    %*** Play button ***
    h{4} = uicontrol(fh,'Style','pushbutton', 'String', 'Play/Pause', 'Position', [1050 400 75 35], 'Callback', @playBtn);
    
    %*** Stop button ***
    h{4} = uicontrol(fh,'Style','pushbutton', 'String', 'Stop', 'Position', [1125 400 50 35], 'Callback', @stopBtn);
    
    %*** Slow button ***
    h{4} = uicontrol(fh,'Style','pushbutton','String','Slow','Position', [1000 400 50 35], 'Callback', @slowBtn);
...
%--function of slow button
function slowBtn(src, ~)
    fh = get(src, 'Parent');
    vs = get(fh, 'UserData');
    if(~vs.vid_playing)
                playVideo(1/3)
